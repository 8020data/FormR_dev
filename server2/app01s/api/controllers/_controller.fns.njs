...
// ----------------------------------------------------------------------------
   ...
   { getControllers:  function( pModule, aShowEm ) {  trace( )                           // called by routes.njs or ${aTable}.controllers.njs
            pDefaultOptions = { Cmd: '', ControllersFilename: '../controllers/_default.controllers.njs' }  // .(10301.03.3)
        if (typeof(pModule) == 'string') { var reModel = new RegExp( `${pModule}.controllers?[^_]\.n?js`)  // .(10316.04.1)
