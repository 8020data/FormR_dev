...
//          APP_HOME        = `${__dirname.replace( /[\\\/]api/, '/api' )}`

      if (__filename == process.mainModule.filename) {                                         // .(10317.04.8 RAM Run stand-alone for testing)
...
//          FORMRs_3        =    `${ process.env.FORMR_HOME }/_3/FR.FNSs/`                        // .(10317.01.1 RAM FormR's Home) 
            FORMRs_4        =    `${ APP_HOME.replace( /[\\/]app[0-9]+[sc]/i, '' ) }/_4/FR.fns02s/` // .(10317.01.1 RAM Server's Home if dynamically assigned) 
                                     jstUtils.setEnv( )                                        // .(10317.03.8 RAM Only necessary if we need process.env vars to be read from .env)
            }
 
        var pFns            =   require( `${APP_HOME}/api/routes/_route.fns.njs` ).fns         // .(10318.02.4 RAM Not so simple from here) 
...
//      var aDB             =  'fr'                                                     // .(10318.01.1 RAM Maybe Want this) 
        ...
        var pModel          =  require( `${APP_HOME}/api/models/index.js` )[ aModel ]   // .(10109.03.1 RAM Make Generic).(10314.08.3).(10318.02.5)
...
//          var aModel      =  require( 'fs' ).readFileSync( `${APP_HOME}/api/models/user.model.json`, 'ASCII' ) // .(10316.02.1 RAM )
            var aModel      =  require( 'fs' ).readFileSync( `${FORMRs_4}/FMR_UserModel1-0.json`,      'ASCII' ) // .(10316.02.1 RAM ).(10318.02.6)
