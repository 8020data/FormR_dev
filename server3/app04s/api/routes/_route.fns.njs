...
//  ------------------------------------------------------------------------------------------
   ...
   { fixTableRoutes: function( aTable, aModel, pRoutes ) { trace( )                     // called by _controller.fns.getControllerRoutes
//          console.log( `fixTableRoutes    [1]  aTable: '${aTable}', aModel: '${aModel}', pRoutes.length: ${ Object.keys(pRoutes).length }`, fmtObj( pRoutes) ); // return
...
//          console.log( `fixTableRoutes    [3]  aTable: '${aTable}', aModel: '${aModel}', pRoutes.length: ${ Object.keys(pRoutes).length }`, fmtObj( pRoutes) ); // return
     return pRoutes
            }
//          ----------------------------------------------------------------------

   , getTableRoutes: function( pTableRoutes, aTable, aCmd ) {  trace( )                 // called by _controller.fns.getControllerRoutes
...
//          ----------------------------------------------------------------------
   ...
   , setTableRoutes( pTableRoutes_, aTable, pApp ) {  trace( )                          // called by routes.njs or ${aTable}.controllers.njs
...
//          ----------------------------------------------------------------------
   ...
   , setRoles( pRoutes, aRole, aChg, aActions ) { trace() 
...
//          ----------------------------------------------------------------------
            ...
            aShowEm      =  ''
            TheApp       =  null
  function  setRoutes(   pTableRoutes,  pApp ) { trace()           
...
//          ----------------------------------------------------------------------
  ...
  function  setRoute( aRoute, aRoles, aController ) { trace()
...
//          ----------------------------------------------------------------------
   ...
   function setRoute_( aProtocol, aMethod, aRoute,  aRoles, aController ) { trace()
//          console.log( `  setRoute[28]  ${aProtocol}://${aMethod}  ${aRoute}  ${aController}` )
        ...
        var aControllerFile =  mController.slice( 0, -1 ).join( '.')
            aControllerFile =  aControllerFile.match( /\.js$/ ) ? aControllerFile : `${aControllerFile}${TheControllersExt}`  // .(10225.02.1 RAM ??)
        var aActionName     =  mController.slice(    -1 ) 

//      var pControllers    =  require( `../Controllers/${aControllerFile}${TheControllersExt}` ).Controllers   //#.(10211.02.x).(10225.02.1)
        ...
        var aControllerFile =  take( 39, aControllerFile.replace( /\.[nm]?js$/, '' ) + '.' + aActionName )                                              // .(10225.02.2 RAM Remove extension)
...
//          ----------------------------------------------------------------------
   ...
   function fmtRoute( aRoute ) { trace(  aRoute )
...
//          ----------------------------------------------------------------------
   ...
   function fmtRoles( aRoles ) { trace( 0 )
            aRoles =  aRoles || ''
...
//          ----------------------------------------------------------------------
   ...
   function getUnqKeys(  pObjs1,    pObjs2, nWdt ) { trace( 0 )
...
//          ----------------------------------------------------------------------
   ...
   function nObjs ( pObj ) { var m = fmtObj( pObj ).match( /{\n/g ); return m ? m.length: 0 }
