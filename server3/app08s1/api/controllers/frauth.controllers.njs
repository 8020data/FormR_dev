...
//    var   aSecret             = 'bezkoder-secret-key'                                                     // .(10227.03.2).(10317.01.1)
      ...
      var   aSalt               =  require( `${FORMRs_3}JWT_Config1-0.njs`).Salt                            // .(10416.04.1 RAM Need this for bcrypt.hash to match).(10419.01.4)
         ...
         Role.findAll( { where: { name: { [Op.or]: req.body.roles } } } )
             ...
             .then( roles => {   user.setRoles( roles )                                                     // .(10312.06.1 RAM This Sequilize method stores each role in {req.body.roles} into the related tables 'user_roles')
                             ...
                             var aMsg   = `User registered successfully for role(s): ${ aRoles }.`
                                 res.send( { message: aMsg } ); trace( aMsg, 2 )  // , -1 );                // .(10228.05.x RAM user roles).(10311.05.5 End).(10406.10.1 RAM Increase level)
                     } );        } );
               ...
               //                    .then( function setRole1( ) {                                          //#.(10228.04.4 RAM Get rid of anonymous function).(10312.06.2)
                             var aMsg   = `User registered successfully for role: ${ pNewUser.role }.`      // .(10312.06.3 RAM Was: .roles[0])
                                 res.send( { message: aMsg } ); trace( aMsg, 2 )  // , -1 );                // .(10406.10.2)
...
, login   : function login( req, res )    { trace( `username: ${req.body.username}`, 2) // .(10228.12.4)

    User.findOne( { where: { username: req.body.username } } )

    .then( function chkUser( user ) {                                                   // .(10228.04.4 RAM Get rid of anonymous function)

        if (!user) {             trace("User Not found.", 1 )                           // .(10228.05.x)
                                 return res.status(404).send( { message: "User Not found." } ); 
                                 }
                                 trace(   `Checking Passwords: '${req.body.password}'`, 1 )         
                                 trace(   `   vs stored in DB: '${user.password}'`    , 1 )         
          var passwordIsValid  = bcrypt.compareSync( req.body.password, user.password   );
         if (!passwordIsValid) {
                                 trace("Invalid Password.", 1 )                         // .(10228.05.x)
...
//                           var aRoles = roles.join( ",")                              //#.(10311.05.3 RAM roles is a array of sequelize objects)
         for (let  i = 0;  i  <  roles.length;  i++) {
              ...
              authorities.push( "ROLE_" + roles[i].name.toUpperCase() );
                                 ...
                                 trace(`Login successful for roles: '${ aRoles.substr(1) }'.`, 1) // .(10228.05.x).(10311.05.2)
*/                                                                                                //#.(10415.05.2 End)
                                 trace(  `Login successful for role: '${ user.role }'.`, 1 )      // .(10415.05.3)
                                 trace(  `      Issued token: '${ token.substr(0,60) }...'`, 1 )  // .(10419.03.1 RAM Let's see it) 
...
//        } ); // user.getRoles(        ).then( function setRoles( roles ) { ... } )    //#.(10415.05.2)
    ...
    .catch( err => {             trace(  "Login failed.", 1 )                           // .(10228.05.x)
                                 res.status( 500 ).send( { message: err.message } );
            } );
...
, session : function session( req, res ) { trace( `Checking Token` )                    // .(10312.12.1 Beg RAM)

       var  aToken = req.headers[ 'x-access-token']

       if (!aToken) {         trace( 'A token is required for this web page', 2 )      // .(10419.01.7 Beg RAM If not given)
            res.status(  500 ).send(   { message: 'A token is required for this web page' } )
                               return
            }                                                                           // .(10419.01.7 End) 
       var  pToken = verify( aToken )
        if (pToken.err) {      trace( `Invalid token: '${ aToken.substr(0,60) }...'`, 1 )
            res.status(  500 ).send(   { message: `Invalid token: '${aToken}'.` } )     // .(10419.01.8 RAM Was: pToken.err)
                               return
            }
       ...
       User.findByPk( pToken.id )
           .then( function chkUserSession( pUser ) {
            ...
            var aMsg        = `This session is validated for ${aUsername} with ${aRole} privileges.`
                ...
                pUser       =  JSON.parse( JSON.stringify( pUser ) )                    // .(10424.05.1 RAM Convert from Sequelize Object to real Object)
                pUser.accessToken  =  aToken                                            // .(10420.02.1 RAM Client: getCurrentUserFromServer needs it).(10424.05.1 RAM But being a Sequelize Object with no .get .set, it doesn't get stringified)
//          res.status(  200 ).send(  aMsg )                                            //#.(10419.01.6)
            res.status(  200 ).send(  JSON.stringify( pUser ) )                         // .(10419.01.6 Was: aMsg. Send User Data instead) 
                               return
                               } )
           .catch( err => {    trace( "Session failed.", 1 )
...
//      var aSecret = 'bezkoder-secret-key'
      try {
        var pToken  =  jwt.verify( aToken, aJWTkey )                           // .(10420.01.1 RAM Was: aSecret)
   } catch( pErr ) {
