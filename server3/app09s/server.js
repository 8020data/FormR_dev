...
//              res.json( { message: `Welcome to FormR Server1 ${aApp} application.` });     // .(01012.02.2)
                res.send(  `<br><h2> &nbsp; Welcome to FormR Server1 ${aApp} application.</h2>`);    // .(10115.01.1)
                } ); // eof app.get( '/', function )
// -----------------------------------------------------------

            app.use( express.static( './docs' ) )
//          app.get( "/docs", ( req, res ) => { res.send( "What's up Doc?") } )                      // .(10115.01.2)
/*          app.get( "/docs", ( req, res ) => {                                                    // .(10115.01.2 Beg)
                res.sendFile( `${__dirname}/docs/index.html` )  
                } )                                                                                  // .(10115.01.2 End)
*/
//          app.use( crud( '/admin/users', sequelizeCrud( User ) ) )   // .(10103.03.1 RAM) 
       var  setAPIroutes  =  require( './api/routes/setAPIroutes.js' ) // .(10106.02.1 RAM Use to set all routes)
            setAPIroutes( app, 'users', 'user.controller.js' );        // .(10106.02.1 RAM i.e. the name of the controller file: user.controller.js)
//          setAPIroutes( app, 'roles', 'role.controller.js' );        // .(10109.01.3 BTR Define Route)
