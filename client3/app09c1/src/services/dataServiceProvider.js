...
//                , baseURL:   'http://' + apiURL + '/api'                      //#.(10315.09.1)
                  , baseURL:   'http://' + apiURL                               // .(10315.09.1)
                    } );
/*
...
// ----------------------------------------------------------------------------------------------------
  ...
  , updateOne:  function( resource, params ) {
              [ resource, params ] = fixParams( arguments, 'one' );
//      const   url   = `/${resource}/${params.id}`;          xDebug( url, params.data, '   PUT' )     //#.(10314.08.11)
        const   url   = `/${resource}/update/${params.id}`;   xDebug( url, params.data, '   PUT' )     // .(10314.08.11 RAM Ok, let's modify the route)
  ...
  , deleteOne:  function( resource, params ) {
              [ resource, params ] = fixParams( arguments, 'one' );
//      const   url   = `/${resource}/${params.id}`;          xDebug( url, params.data, 'DELETE' )     //#.(10314.08.12)
        const   url   = `/${resource}/delete/${params.id}`;   xDebug( url, params.data, 'DELETE' )     // .(10314.08.12)
...
//  ------------------------------------------------------------------------------------------------------------------------------------
      ...
      function  fixParams( mArgs, aMethod ) {
            ...
            var aTable  =  mArgs[0]  ? mArgs[0] : '', pParams

            if (aMethod === 'new'  ) { pParams =  mArgs[1] }
            if (aMethod === 'all'  ) { pParams = { }       }
            if (aMethod === 'one'  ) { pParams = (mArgs[1] && mArgs[1].id)  ? mArgs[1] : { id:  mArgs[1] }
//                                     pParams =  pData ? { ...pParams, ...pData } : pParams
                                       }
            if (aMethod === 'many' ) { pParams = (mArgs[1] && mArgs[1].ids) ? mArgs[1] : { ids: mArgs[1] }
//                                     pParams =  pData ? { ...pParams, ...pData } : pParams
                                       }
