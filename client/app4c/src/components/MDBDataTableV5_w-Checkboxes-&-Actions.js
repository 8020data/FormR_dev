...
export default function WithMultipleCheckboxes() { // datatable = { columns, rows }, checkbox1, setCheckbox1, showLogs2, return ( )
 ...
 const pRows = [ 
  ...
  { columns: 
     [ { label: 'Name',               field: 'name',     width: 175, attributes: { 'aria-controls': 'DataTable', 'aria-label': 'Name', } }
     , { label: 'Position',           field: 'position', width: 300  }
     , { label: 'Office',             field: 'office',   width: 200  }
     , { label: 'Age',                field: 'age',      width: 100, sort: 'asc',     }
     , { label: 'Start date',         field: 'date',     width: 150, sort: 'disabled' }
     , { label: 'Salary',             field: 'salary',   width: 100, sort: 'disabled' }
     , { label:  mDropDowns[1]( '' ), field: 'actions',  width: 250, sort: 'disabled' }      // .(01204.07.2)

       ]           // eoa columns
//, rows: pRows    // eoa rows
  , rows: pRows.map( fmtRow )    // eoa rows
  ...
  return (
    < >
      <MDBDataTableV5
       ...
       <Result> { checkbox1 && <p> { JSON.stringify( delete checkbox1.checkbox &&
//                                                   checkbox1                    
//                                                   checkbox1.map( pRec => pRec.name ) 
                                                     checkbox1.map( pRow => 
                                                         pRow.checkbox.props.id + ": " 
                                                       + pRow.name )
                                                     ) } </p> } </Result> 
    < />
